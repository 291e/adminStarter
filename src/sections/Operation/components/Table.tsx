...
type Props = {
  ...
  onRegisterIndustrialAccident?: (row: RiskReport) => void;
};
    ...
    >
      {rows.map((row, index) => {
        ...
        const key = row.riskReportIdx
          ? String(row.riskReportIdx)
          : row.id || `risk-report-${index}`;
        const companyName = row.companyName || '미지정 조직';
              ...
              >
                <Box sx={{ position: 'relative', height: '100%' }}>
                  ...
                  <Stack spacing={0.5} sx={{ mb: 0.5 }}>
                    ...
                    <InfoRow label="보고자" value={row.reporterName || '-'} />
                    <InfoRow label="작성자" value={row.authorName || '-'} />
                  </Stack>
                ...
                >
                  <MenuList>
                    <MenuItem
                      onClick={(e) => {
                        ...
                        if (import.meta.env.DEV) {
                          console.log('🔍 [Table] 수정 메뉴 클릭', { row, key });
                        }
                        onEdit?.(row);
                      }}
                    >
                    ...
                    <MenuItem
                      onClick={() => {
                        handleCloseMenu(key);
                        onRegisterIndustrialAccident?.(row);
                      }}
                    >
                      산업재해 등록
                    </MenuItem>
